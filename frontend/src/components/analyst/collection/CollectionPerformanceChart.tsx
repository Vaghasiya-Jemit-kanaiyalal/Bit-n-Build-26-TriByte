...
export const CollectionPerformanceChart: React.FC<CollectionPerformanceChartProps> = ({ data }) => {
  return (
    <div className="bg-white border border-slate-200/80 rounded-2xl p-5 shadow-2xs">
      <div className="flex items-center justify-between mb-4">
        <div>
          <h3 className="text-sm font-bold text-slate-900">Collection Performance Trend</h3>
          <p className="text-xs text-slate-500">Daily executed collection volume & on-time stops</p>
        </div>
      ...
      <div className="h-48 flex items-end justify-between gap-3 pt-4 border-b border-slate-100">
        {data.map((item, idx) => {
          ...
          const onTimeH = (item.onTime / 220) * 140;
          return (
            <div key={idx} className="flex-1 flex flex-col items-center gap-2 group h-full justify-end">
              <div className="w-full flex items-end justify-center gap-1.5 h-36">
                <div
                  style={{ height: `${compH}px` }}
                  className="w-3.5 bg-blue-600 rounded-t-sm group-hover:bg-blue-700 transition-all relative"
                >
                ...
                <div
                  ...
                  className="w-3.5 bg-emerald-500 rounded-t-sm group-hover:bg-emerald-600 transition-all relative"
                >
                  <span className="opacity-0 group-hover:opacity-100 absolute -top-6 left-1/2 -translate-x-1/2 bg-slate-900 text-white text-[10px] font-bold py-0.5 px-1.5 rounded whitespace-nowrap transition-opacity z-10">
                    {item.onTime}
                  </span>
